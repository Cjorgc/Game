...
var Level = class Level {
    constructor(plan) {
        ...
        this.startActors = [];this.rows = rows.map((row, y) => {
            return row.map((ch, x) => {
...
const levelChars = {".": "empty", "#": "wall", "+": "lava","@": Player, "o": Coin,"=": Lava, "|": Lava, "v": Lava, "M": Monster};
...
function drawGrid(level) {
    return elt("table", {class: "background",style: `width: ${level.width * scale}px`}, ...level.rows.map(row =>elt("tr", {style: `height: ${scale}px`},...row.map(type => elt("td", {class: type})))));
}
...
function runLevel(level, Display) {
    ...
    let ending = 1;
    let running = "yes"
    let keys = trackKeys(["ArrowLeft", "ArrowRight", "ArrowUp"]);

    

    return new Promise( (resolve) => {

        function escpHandler(event){
        ...
        function frame(time){
            ...
            window.addEventListener('keydown', escpHandler);
            ...
            if (state.status == "playing") {
                ...
                display.clear();
                window.removeEventListener('keydown', escpHandler)
                keys.unregister()
